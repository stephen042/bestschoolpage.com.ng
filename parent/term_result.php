...
foreach($aryDetail as $iList) {
	...
	array_push($iLeagueId,$iManageStudents['student_id']); 
                        ...
                        <tr>
                          ...
                          <td class="sectsab <?php if($iManageStudents['student_id']==$_GET['student_id']) { echo "active"; } ?>">
						...
						if($iManageStudents['picture']=='') { ?>
                                ...
                                <?php  $db->getVal("select session from school_session where id='".$iManageStudents['session']."' and create_by_userid='".$create_by_userid."'"); ?><br>
								 <?php  $db->getVal("select term from school_term where id='".$iManageStudents['term_id']."' and create_by_userid='".$create_by_userid."'"); ?> <br>
								 <?php  $db->getVal("select short_name from school_class where id='".$iManageStudents['class']."' and create_by_userid='".$create_by_userid."'"); ?>
              ...
              <form method="post" action="">
                ...
                <label>Ass.:</label>
                <?php $assesment=$db->getRows("select * from school_assessment where create_by_userid='".$studentdata['create_by_userid']."' and class_id='".$_POST['class_id']."' order by id desc "); 
...
<input type="checkbox"  name="assesment[]" value="<?php echo $iShoD['Assignmentid']; ?>" <?php if(in_array($iShoD['Assignmentid'],$_POST['assesment'])) { echo "checked"; } if($_POST['assesment']=='') { echo 'checked'; }  ?>>
              ...
              <form action="" method="post">
                ...
                <a   onClick="movetermsresult()" class="printer_pdf" id="print_pdfnew" target="_blank">Print Report Sheet</a></span>
              </form>
	...
	?>
                ...
                <center>
                  <p><?php  $db->getVal("select name from school_class where id='".$currentStudent['class']."'"); ?></p>
                </center>
                <div class="zasw1">
                    ...
                    <table  class="table table-striped table-bordered">
                      <thead>
                        <tr>
                          ...
                          <th><?php  echo $db->getVal("select assesment from school_assessment where id='".$Val."'").' '.$db->getVal("select percentage from score_entry_time_frame where assesment_id='".$Val."'");  ?></th>
                          <?php } ?>
                          <th>Total</th>
                          <th>Grade</th>
                          <th> Average</th>
                      ...
                      <tbody>
                        <?php   	$subjectdetail=$db->getRows("select * from school_subject where class_id = '" .$_POST['class_id']. "' and create_by_userid='".$studentdata['create_by_userid']."'");
							         $tSub=0; 	 foreach ($subjectdetail as $Ilist) {   $tSub++;  ?>
                        <tr>
                          <td><?php echo $Ilist['subject']; ?></td>
                          <?php 	$count=0; 	foreach($_POST['assesment'] as $Val)  {  	$count=$count+1;
										
$score=$db->getRow("select score , student_id from input_score_class_teacher where class_id = '" .$_POST['class_id']. "' and assesment_id='".$Val."' and student_id IN ($iCommaSeparatedStudent) and subject_id='".$Ilist['id']."' and  session_id='".$_POST['session']."' and term_id='".$_POST['term_id']."' and create_by_userid='".$studentdata['create_by_userid']."'");
if($score['student_id']!='')
	{
...
?>
                          ...
                          <td><?php
										...
										?></td>
                          <td><?php 
										echo  $gradding=$db->getVal("select grade from school_grade where create_by_userid='".$studentdata['create_by_userid']."' and minimum_number <= ".$grandTotal." and maximum_number >= ".$grandTotal."");
										?></td>
                          <td><?php
										$a=$Ilist['id'];
...
function movetermsresult()
	{	 window.open('<?php echo PARENT_URL; ?>term_result_pdf.php?randomid=<?php echo $iFetchRandomId; ?>&student_id=<?php echo $_GET['student_id']; ?>&session=<?php echo $_POST['session']; ?>&term_id=<?php echo $_POST['term_id']; ?>&assesments=<?php echo $assesments; ?>', '_blank');
...
if(randomid=='')
	{
		document.getElementById("print_pdfnew").style.display='none';	
	}	
</script>
