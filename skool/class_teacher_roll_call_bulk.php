...
if (isset($_POST['roll_call']) || isset($_POST['update_roll_call'])) {
    ...
    if ($validate->validate() && count($stat) == 0) {	
        ...
        $total_days_open=$_POST['total_days_open'];
        $class_id=$_POST['class_id'];
        $student_id=$_POST['student_id'];
        
		foreach($student_id as $key=>$val)
		{
		    if($present[$i] > $total_days_open[0]){
		        $_SESSION['error'] = "Error! 'Days Present' cannot be greater than 'Total days open'!";
		        redirect($FileName.'?action=table&randomid='.$_GET['randomid'].'&date='.$_GET['date'].'&session='.$_GET['session'].'&term_id='.$_GET['term_id'].'&class_id='.$_GET['class_id']);
                   ...
                   $randomid=randomFix(15);
                    $aryData = array(
        				...
        				'randomid'                =>  $randomid, 
                        'present'                 =>  $present[$i],
                        'absent'                  =>  $total_days_open[0] - $present[$i],
                        'total_days_open'         =>  $total_days_open[0],
                         
                    );
                    $student_ids .= " - " . $val;
                    $query = "select * from  class_teacher_roll_call_bulk where student_id='".$val."' and session_id='".$_GET['session']."' and  term_id='".$_GET['term_id']."' and  class_id='".$class_id."'";
                    $queries .= " ----- " .$query;
                    $data = $db->getRows($query);
                    $countData = 0;
                    foreach ($data as $data1) $countData += 1;
                    if($countData == 0){
        			    $created += 1;
        			    $flgIn = $db->insertAry("class_teacher_roll_call_bulk", $aryData);	 
                    }else{
        			    $updated += 1;
        		        $flgIn = $db->updateAry("class_teacher_roll_call_bulk", $aryData, "where student_id='".$val."' and session_id='".$_GET['session']."' and  term_id='".$_GET['term_id']."' and  class_id='".$class_id."'");
        			}
        		}
        ...
        //		$_SESSION['success'] = "Class Attendance Updated Successfully";
         		$_SESSION['success'] = "Class Attendance Submitted Successfully -- updated =" .$updated . " -- created = ".$created ." -- studentids = ".count($student_id);
        // 		 . " -- session_id = ". $_GET['session'] . " -- term_id = ". $_GET['term_id'] . " -- flgIn = ". $flgIn . " -- countData = ". $countData;
        		redirect($FileName.'?action=table&randomid='.$_GET['randomid'].'&date='.$_GET['date'].'&session='.$_GET['session'].'&term_id='.$_GET['term_id'].'&class_id='.$_GET['class_id']);
          ...
          <div class="sectionza">
            <div class="col-md-12   col-xm-12">
              ...
              <form method="GET" action="">
                ...
                <input type="submit" value="Load record" name="" />
              </form>
              <?php } ?>
              <?php if ($_GET['action']=='table') { 
								//$rollDetail=$db->getRow("select * from class_teacher_roll_call_bulk where session_id='".$_GET['session']."' and term_id='".$_GET['term_id']."' and class_id='".$studentDetail['id']."' and create_by_userid='".$create_by_userid."'");
                    ...
                    <div class="panel with-nav-tabs panel-success">
                      <div class="panel-heading">
                        <div class="topside-section">
                          ...
                          <div class="card-box table-responsive tablthisresponsive">
                            <table  class="table table-striped table-bordered tablthisresponsive">
                              ...
                              <?php
								...
								$rollDetail=$db->getRow("select * from class_teacher_roll_call_bulk where  student_id='".$iList['student_id']."' and session_id='".$_GET['session']."' and term_id='".$_GET['term_id']."' and class_id='".$studentDetail['id']."' and create_by_userid='".$create_by_userid."' LIMIT 1");
								?>
                              <tr class="<?php if($i%2=='0'){echo "info";}else{echo "success";}?>" >
                                ...
                                <input type="hidden" name="total_days_open[]" class="total_days_open" value="<?php echo $rollDetail['total_days_open']; if($i == 1) $total_days_open = $rollDetail['total_days_open']; ?>" />
                                 <input class="present" onchange="updateAbsent(this)" onkeyup="updateAbsent(this)" min="0" max="365" style="background: #fff;text-align: center;" type="number" name="present[]" value="<?php echo $rollDetail['present']; ?>" required/></td>
                                <td style="text-align: right;">
                                    <input min="0" class="disabled absent" max="365" style="background: #e6e4e4;text-align: center;" type="text" name="absent[]" value="<?php echo $rollDetail['absent']; ?>" disabled/></td>
                              </tr>
                              <?php } ?>
                                </tbody>
                          ...
                          <?php if($i > 0){?>
                          <div class="d-block" style="background: #dff0d8;margin-bottom: 20px;text-align: right;padding-right: 15px;">Total Days School Open:
                          <input id="total_days" type="number" min="1" max="365" style="background: #fff;text-align: center;" value="<?php echo $total_days_open; ?>" onchange="updateDaysOpen(this)" onkeyup="updateDaysOpen(this)" required> </div>
                          <div class="d-block" style="text-align: right;padding-right: 15px;">
                            <button type="submit" name="update_roll_call" class="gwt-Button"> Submit Attendance </button>
                          </div>
                          <?php } ?>
                          <Br>
                          <Br>
...
//update panels
  ...
  <script>
    function updateDaysOpen(el){
        let d = el.value;
        for(let a of $(".total_days_open").toArray()){
            a.value = d;
        }
        for(let b of $(".present").toArray()){
            updateAbsent(b);
        }
    }
    function updateAbsent(el){
        let present = el.value.trim();
        let total = $("#total_days").val().trim();
        if(present && !isNaN(present) && total && !isNaN(total)){
            let absent = total - present;
            let absentEl = $(el).parent().parent().find('.absent').first();
            if(absent < 0) absent = 'invalid';
            $(absentEl).val(absent);
        }
    }
