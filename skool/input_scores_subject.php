...
<?php include('inc.sideleft.php'); ?>
  ...
  <div class="row" style="overflow:scroll; height:auto;">
  <div class="sectionza">
  <div class="col-md-12">
  <div class="col-md-4">
    <div class="zasw ">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="sectsab <?php if($_GET['randomid']==$iList['randomid']) { echo "active"; }?>"><a href="<?php echo $FileName; ?>?action=input_score&randomid=<?php echo $iList['randomid']?>">
                <ul>
                  <li> <span class="zwq"> <img class="table-img" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTbRnETjp3LR55_QTrkge0ZW1VZwhnBGrZuDDM4DSh6dQSMFG21"></span> <?php echo $iList['subject'];?><br>
  ...
  <thead>
    <tr>  <!--- <th>offering</th>-->
				...
				<?php $scoredetail=$db->getRow("select * from input_score_class_teacher where assesment_id='".$assesmentId['id']."' and subject_id='".$subjectid['id']."' and create_by_userid='".$create_by_userid."' and class_id='".$aryList['id']."' ");
					if($scoredetail['id']=='') { ?>
					<form method="post" action="">
                      <tbody>
						...
						?>
                          <tr>
                           <!---<td>
								<input type="checkbox" name="offering[]" value="1" <?php if($_POST['offering']=="1") { echo "checked"; }?>></td>
						    -->
							...
							<td><input type="number" class="form-control" value="<?php echo $_POST['score']; ?>" name="score[]" max="<?php echo $svevalue = trim($iPercentage,'%'); ?>" min="0"  step="any" required > <!-- <span style="color:red;">maximum Input<?php echo $svevalue = trim($iPercentage,'%'); ?></span>---> </td>
						  </tr>
					<?php } ?>
						</tbody>
					...
					<?php } else { 
					$scoredetail=$db->getRow("select * from input_score_class_teacher where assesment_id='".$assesmentId['id']."' and subject_id='".$subjectid['id']."' and create_by_userid='".$create_by_userid."' and class_id='".$aryList['id']."' ");
					...
					{
					$scoredetaill=$db->getRow("select * from input_score_class_teacher where assesment_id='".$assesmentId['id']."' and subject_id='".$subjectid['id']."' and create_by_userid='".$create_by_userid."' and class_id='".$aryList['id']."' and student_id='".$iList['id']."' ");
					?>
					<tr>
                    <!--- <td>
					<input type="checkbox" name="offering[]" value="1" <?php if($scoredetaill['offering']=="1") { echo "checked"; }?>>
                    </td>-->
					...
					<td><input type="number" class="form-control" value="<?php echo $scoredetaill['score']; ?>" name="score[]" max="<?php echo $upvalue = trim($iPercentage,'%'); ?>" min="0"  step="any" required ><!--<span style="color:red;">maximum Input <?php echo $upvalue = trim($iPercentage,'%'); ?> </span>--></td>
					</tr>
					<?php } ?>
