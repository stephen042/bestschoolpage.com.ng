...
$traitsId=$db->getRow("select * from  manage_traits where randomid='".$_GET['trait']."' and create_by_userid='".$create_by_userid."'");
...
$allSutdent=$db->getVal("select count(id) from manage_student  where class='".$aryList['id']."' and session='".$sessionId['id']."' and term_id='".$term_id['id']."'and create_by_userid='".$create_by_userid."'");

if(isset($_POST['add_trait']))  
{   
    
    if($allSutdent==$_POST['totalStudent'])
	{
 
    $validate->addRule($traitsId['id'],'','Trait',true);
	...
	if($validate->validate() && count($stat)==0)
	{
		foreach($_POST['student_id'] as $key=>$val)
		{
			...
			$aryData = array(
							...
							'term_id'					    =>	$term_id['id'],
							'class_id	'					=>	$aryList['id'],
							'traits_id'			     		=>	$traitsId['id'],
							'student_id'					=>	$_POST['student_id'][$key],
							'trait'							=>	$_POST['traits'][$key],
				...
				$flgIn2 = $db->insertAry("student_traits_class_teacher", $aryData);
	...
	{   $remainingStudent=$allSutdent-$_POST['totalStudent'];
		$stat="$remainingStudent entry not filied ";
	}
...
if(isset($_POST['edit_traits']))  
{  
	
	$flgIn1 = $db->delete("student_traits_class_teacher", "where session_id='".$sessionId['id']."' and term_id='".$term_id['id']."' and class_id='".$aryList['id']."' and traits_id='".$traitsId['id']."'");
			...
			<div class="col-md-4">
			   ...
			   <select name="session" id="session"  required>
				...
				<select name="trait">
					...
					<?php  $subjectdetail=$db->getRows("select * from manage_traits where create_by_userid='".$create_by_userid."'");
					
							foreach ($subjectdetail as $Ilist) { 
					?>
							
					<option value="<?php echo $Ilist['randomid']; ?>" <?php if($Ilist['randomid']==$_GET['trait']) { echo "selected"; } ?> > 
					<?php echo $Ilist['trait']; ?> </option>
...
<?php if($_GET['action']=='input_score') { 
  ...
  $traitss=$db->getRow("select * from manage_traits where randomid='".$_GET['trait']."' and create_by_userid='".$create_by_userid."'"); 
  ?>
                          ...
                          <table  class="table table-striped table-bordered tablthisresponsive">
                            <thead>
                              <tr>
                                 ...
                                 <th>First Name</th>
								 <th>Last Name</th>
								 <th><?php echo $traitss['trait']; ?></th>
							  </tr>
                            </thead>
				<?php $traitsDetilas=$db->getRow("select * from student_traits_class_teacher where traits_id='".$traitss['id']."' and create_by_userid='".$create_by_userid."' and session_id='".$sessionId['id']."' and term_id='".$term_id['id']."' and class_id='".$aryList['id']."' ");
					if($traitsDetilas['id']=='') { ?>
					
                      <tbody>
                        <?php $i=0;
					
						$aryList11=$db->getRows("select * from manage_student  where  class='".$aryList['id']."' and session='".$sessionId['id']."' and term_id='".$term_id['id']."' and create_by_userid='".$create_by_userid."' order by first_name asc");
						...
						?>
                          <tr>
							...
							<td><input type="text" style="text-transform: capitalize;" class="form-control" value="<?php echo $_POST['traits']; ?>" name="traits[]" required> </td>
						  </tr>
			<?php       } ?>
						</tbody>
									
                        </table>
						<input type="hidden" name="totalStudent" value="<?php echo $totalStudent; ?>" />
						<input type="submit" name="add_trait" value="SAVE" class="btn btn-primary"  style="color:white;"> 
					</form>
					<?php } else { 
				...
				<form method="post" action="">
                   <tbody>
           <?php $i=0;
		$aryList12=$db->getRows("select * from manage_student  where class='".$aryList['id']."' and session='".$sessionId['id']."' and term_id='".$term_id['id']."' and create_by_userid='".$create_by_userid."' order by first_name asc");
	       $totalStudent=0;
	foreach($aryList12 as $iLists)
	
	{
		$totalStudent++;
		$traitsdetaill=$db->getRow("select * from student_traits_class_teacher where traits_id='".$traitsId['id']."'  and create_by_userid='".$create_by_userid."' and class_id='".$aryList['id']."' and student_id='".$iLists['id']."' ");
	 ?>
     <tr>
		<td><?php echo $iLists['student_id']; ?>
		<input type="hidden" name="student_id[]" value="<?php echo $iLists['id']; ?>">   </td>
		<td><?php echo $iLists['first_name']; ?></td>
		<td><?php echo $iLists['last_name']; ?> </td>
		<td><input type="text" style="text-transform: capitalize;" class="form-control" value="<?php echo $traitsdetaill['trait']; ?>" name="traits[]" required ></td>
	 </tr>
		<?php } ?>
		...
		<input type="hidden" name="totalStudent" value="<?php echo $totalStudent; ?>" />
	<input type="submit" name="edit_traits" value="UPDATE" class="btn btn-primary"  style="color:white;"> 
	</form>
<?php } ?>
</div>
</div>
