...
foreach($_POST['student_id'] as $key => $val) {	
	...
	$aryData = array(
					...
					'no_of_subject'     	 	         	    =>	$_POST['no_of_subject'][$key],
					'totalScore'     	 	         			=>	$_POST['totalScore'][$key],
					'position'     	 	         			    =>	$_POST['position'][$key],
					'comments'     	 	         			    =>	$_POST['comments'][$key],
					
					);

        $flgIn2 = $db->insertAry("principle_remarks", $aryData);
	 ...
	 $aryData = array(
        ...
        $flgIn2 = $db->updateAry("principle_remarks", $aryData,"where id='".$_POST['primaryid'][$key]."'");
                    ...
                    <table id="example" class="display" >
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td class="sectsab <?php if($_GET['randomid']==$iList['randomid']) { echo "active"; }?>"><a href="<?php echo $FileName; ?>?action=basic_info&randomid=<?php echo $iList['randomid']?>">
                            <ul>
              ...
              <div class="col-md-8 col-xs-12">
                <div class="zasw1">
                  ...
                  <div class="card-box  table-responsive tablthisresponsive">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <?php $totalscore = $db->getRow("select SUM(score) as total_score from input_score_class_teacher where class_id='".$classDetail['id']."' and subject_id='".$iListDetail['id']."' and student_id = '" .$aryStudent['id']. "' and create_by_userid='".$create_by_userid."'");
									 $totaslscore=$totalscore['total_score'];
                          ...
                          <td id="<?php echo $a; ?>"></td>
                          <td><?php	
								$commentDetails=$db->getRow("select * from principle_remarks  where student_id='".$aryStudent['id']."' and create_by_userid='".$create_by_userid."'");
                                 ?>
                           
                              <input type="hidden" value="<?php echo $aryStudent['id']; ?>" name="student_id[]">
                              <input type="hidden" value="<?php  echo $totalSub; ?>" name="no_of_subject[]">
                              <input type="hidden" value="<?php echo $grandTotal; ?>" name="totalScore[]">
                              <input type="hidden" value="" id="stu_<?php echo $a; ?>" name="position[]">
                              <input type="hidden"   value="<?php echo $commentDetails['id']; ?>"  name="primaryid[]">
                              <textarea name="comments[]"><?php echo $commentDetails['comments']; ?></textarea>
                           </td>
                        </tr>
                        <?php } ?>
                      </tbody>
                    ...
                    <table>
                    <tr><td> <button class="sectsab active btn btn-primary" type="submit" name="submitcomment"   style="color:#fff;">Submit Comment</button></td></tr>
                    </table>
                     </form>
                  </div>
                </div>
...
foreach ($rankedScores as $studentwa => $data) {
	...
	echo "<script>document.getElementById('stu_$studentwa').value=$rank;</script>";
