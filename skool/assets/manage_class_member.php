...
<!-- Start content -->
<div class="content">
<span id="errormsg"><?php echo msg($stat);?></span>
<div class="container">
<!-- Page-Title -->
<div class="row">
<div class="col-sm-12">
...
<tbody>
<?php $aryDetail=$db->getRows("select * from school_class"); 
...
<?php } ?>			
   ...
   <?php 
         $classDetails=$db->getRow("select * from school_class where randomid='".$_GET['randomid']."'"); 
         $sessionDetails=$db->getRow("select * from school_session where id='".$classDetails['session_id']."'");
		 $staffDetails=$db->getRow("select * from class_teacher where school_class='".$classDetails['id']."'"); 
		 $staffName=$db->getRow("select * from staff_manage where staff_id='".$staffDetails['staff_id']."'");
	?>
    <div class="col-md-9">
    ...
    <div class="zasw1">
<div class="">
<span><h4>Session:<?php echo $sessionDetails['session'];?></h4></span>
<span><h5>Class: <?php echo $classDetails['name'];?></h5></span><hr>
<span><h4>Teacher's Name:<?php echo $staffName['first_name']." ".$staffName['last_name'];?>(<?php echo $staffName['staff_id'];?>)</h4></span>
</div>
...
<a href=""onclick="return confirm('Select a class to proceed')">Add Member(s)</a>
<?php } else { ?>
<a href="<?php echo SKOOL_URL; ?>manage_class_member.php?action=addMember&randomid=<?php echo $_GET['randomid'];?>">Add Member(s)</a>
<?php } ?>
</li>
<!--<li><a href="<?php echo SKOOL_URL; ?>manage_class_member.php?action=importAnotherClass">Import from Another Class</a></li>-->
<!--<li><a href="<?php echo SKOOL_URL; ?>manage_class_member.php?action=importPromotedRepeated">Import Promoted/Repeated</a></li>-->
<!--<li><a href="<?php echo SKOOL_URL; ?>manage_class_member.php?action=removeMember">Remove Member(s)</a></li>-->
<!--<li><a href="<?php echo SKOOL_URL; ?>manage_class_member.php?action=Transfer">Transfer</a></li>-->
</ul>
</div>

<?php if($_GET['action']=='addMember') { ?>
...
<div class="panel with-nav-tabs panel-success">
	...
	<div style="float:right;margin:12px 219px 0 0;">
<h5>Select Session Of Admission:</h5>
    <select class="form-control" name="student_session" id="student_session" onchange="studentList()">
		  <option value="">select session of Admission</option> 
		  <?php 
		  $sessionDetails=$db->getRows("select * from  school_session order by id desc"); 
           foreach($sessionDetails as $iList)
			  {
		  ?>
		  <option value="<?php echo $iList['id']; ?>"><?php echo $iList['session']; ?></option>
		<?php } ?>
	</select>
	</div> 
...
<ul>
<li>
<input type="button" class= "anch_button" onclick="addClassMember()" value="Add selected student(s)"/>
</li>

<li><a href="<?php echo SKOOL_URL; ?>manage_class_member.php" class= "anch_button" style="height:38px;">Cancel</a></li>
</ul>
</div>
</form>
	<?php } else { ?>
								...
								<tr class="middle">
									...
									<td class="middleCenter">
										<div class="middleCenterInner">
												...
												<?php
									$aryDetail=$db->getRows("select * from class_member where class_id='".$classDetails['id']."'"); 
...
<script>
  ...
  var student_session= document.getElementById("student_session").value;
...
function addClassMember()
    {
		...
		var student_id = document.getElementsByClassName("memberList");
		var school_session = document.getElementById("student_session").value;
		for (var i = 0; i < student_id.length; i++)
		{
		 if (student_id[i].checked)
		   {
		     selected.push(student_id[i].value);
		   }
		}

		var stu_iid=selected.join(",");
      	$.post("class_Ajax.php",
		{
		action:"addMember",
		stu_iid:stu_iid,
		school_session:school_session,
		class_id:'<?php echo $currentClass['id'] ; ?>',   	 
		},
		function(data)
		{ 
