  ...
  <tbody style="font-family: sans-serif;">
    <tr>
      <td style="width:100%;" ><table style="width:100%;">
          <tr>
            <td ><b>Name:</b></td>
            <td ><?php echo $iStudent['first_name'].' '.$iStudent['other_name'].' '.$iStudent['last_name'];?></td>
            <?php if($iSchoolPDFsetting['is_grade']=='1') { ?>
            <td ><b>Final Grade:</b></td>
            <?php  ?>
            <td ><?php echo $graddingg=$db->getVal("select grade, description, comment from school_grade where create_by_userid='".$create_by_userid."' and minimum_number <= ".$ownAvg." and maximum_number >= ".$ownAvg."");?></td>
            <?php } ?>
            <td ></td>
            <td ></td>
          </tr>
          ...
          <tr>
            ...
            <td ><b>Session:</b></td>
            <td ><?php echo $iSession['session']; ?></td>
            <?php } ?>
            <?php if($iSchoolPDFsetting['is_finalaverage']=='1') { ?>
            <td ><b>Final Average:</b></td>
            <td ><?php echo round($ownAvg,2); ?></td>
            <?php } else {  ?>
            <td ></td>
            <td ></td>
            <?php } ?>
          ...
          <tr>
            ...
            <td ><b>Term:</b></td>
            <td ><?php echo $_GET["term_id"] == 'all' ? "All" : $term_id['term']; ?></td>
            <?php } ?>
            <?php if($iSchoolPDFsetting['is_highestaverage']=='1') { 
            ?>
            <td ><b>Highest Avg. in Class:</b></td>
            <td ><?php echo round($highestAvg,2); ?></td>
            <?php } ?>
            <?php if($iSchoolPDFsetting['is_lowestaverage']=='1') { ?>
            <td ><b>Lowest Avg. in Class:</b></td>
            <td ><?php echo round($lowestAvg,2); ?></td>
            <?php } else {  ?>
            <td ></td>
            <td ></td>
            <?php } ?>
          ...
          <tr>
            ...
            <td ><b>Days School Open:</b></td>
            <td ><?php echo $db->getVal("select total_days_open from class_teacher_roll_call_bulk where student_id='".$iStudent['student_id']."' and session_id='".$_GET['session']."' $term_select and class_id='".$iStudent['class']."' and create_by_userid='".$create_by_userid."'"); ?></td>
            <?php } ?>
            <?php if($iSchoolPDFsetting['is_daypresent']=='1') { ?>
            <td ><b>Day(s) Present:</b></td>
            <td ><?php echo $db->getVal("select present from class_teacher_roll_call_bulk where student_id='".$iStudent['student_id']."' and session_id='".$_GET['session']."' $term_select and class_id='".$iStudent['class']."' and create_by_userid='".$create_by_userid."'"); ?></td>
            <?php } ?>
            <?php if($iSchoolPDFsetting['is_dayabsent']=='1') { ?>
            <td ><b>Day(s) Absent:</b></td>
            <td ><?php echo $db->getVal("select absent from class_teacher_roll_call_bulk where student_id='".$iStudent['student_id']."' and session_id='".$_GET['session']."' $term_select and class_id='".$iStudent['class']."' and create_by_userid='".$create_by_userid."'"); ?></td>
            <?php } else {  ?>
            <td ></td>
            <td ></td>
            <?php } ?>
