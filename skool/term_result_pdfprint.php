...
$iParent = $db->getRow("select * from school_register where id='".$create_by_userid."'"); 
...
$iSession=$db->getRow("select * from school_session where id='".$_GET['session']."' and create_by_userid='".$iParent['id']."' ");
$term_id=$db->getRow("select * from school_term where id='".$_GET['term_id']."' and create_by_userid='".$iParent['id']."' ");
...
$scoresTotal=$db->getRow("select * from input_score_class_teacher where  session_id = '" .$_GET['session']. "' and term_id = '".$_GET['term_id']."' and assesment_id='".$Val."' and student_id IN ($iCommaSeparatedStudent) and subject_id='".$IlistTotal['id']."' and create_by_userid='".$create_by_userid."'");
				...
				<table style="width:100%;">
					<tr>
					  ...
					  <?php	$ggrade=$grandTotalsss/$tSubtotal; ?>
						<td style=""><?php echo $graddingg=$db->getVal("select grade from school_grade where create_by_userid='".$create_by_userid."' and maximum_number > ".$ggrade." or maximum_number = ".$ggrade."");?> </td>
                           <?php } ?>
...
$scoreNew=$db->getRow("select * from input_score_class_teacher where assesment_id IN($assesmentIn) and student_id='".$iStudent['id']."'  and create_by_userid='".$create_by_userid."'");
					...
					<tr>
						...
						<td style="">	
						<?php echo $totalOpen=$db->getVal("select count(id) from class_teacher_roll_call where session_id='".$_GET['session']."' and term_id='".$_GET['term_id']."' and class_id='".$iStudent['class']."' and create_by_userid='".$create_by_userid."'"); ?>
                       </td>
                        <?php } ?> 
	...
	<tbody style="font-size: 18px;font-weight: bolder;font-family: sans-serif;">
		...
		<tr style="height: 35px;">
			...
			foreach($totalAssesment as $Val) 
			{ 	$count=$count+1;
				$scores=$db->getRow("select * from input_score_class_teacher where  term_id ='".$_GET['term_id']."'  and session_id ='".$_GET['session']."'  and assesment_id='".$Val."' and student_id IN ($iCommaSeparatedStudent) and subject_id='".$Ilist['id']."' and create_by_userid='".$create_by_userid."'");
				?>
			...
			<?php echo $outOf=$db->getVal("select count(id) from  manage_student where session='".$_GET['session']."' and term_id='".$_GET['term_id']."' and class='".$iStudent['class']."' and create_by_userid='".$create_by_userid."'"); ?>
			</td>
			...
			<td style="font-weight: 100; text-align: center;  border: 1px solid #000000;">
            <?php echo $db->getVal("select description from school_grade where create_by_userid='".$create_by_userid."' and maximum_number > ".$grandTotal." or maximum_number = ".$grandTotal."");?>   
			</td>
		</tr>
		<?php } ?>
	</tbody>
